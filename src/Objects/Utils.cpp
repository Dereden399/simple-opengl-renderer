...
glm::mat4 Rotatable::getRotationMatrix(glm::vec3 angles) {
  return glm::mat4_cast(glm::quat(glm::vec3(
      glm::radians(angles.x), glm::radians(angles.y), glm::radians(angles.z))));
}

glm::mat3 Rotatable::getRotationMatrix3x3() {
  return glm::mat3_cast(
      glm::quat(glm::vec3(glm::radians(_rotation.x), glm::radians(_rotation.y),
...
void Rotatable::faceTo(const glm::vec3& dir) {
  ...
  float rotationX =
      glm::degrees(atan2(-direction.y, sqrt(direction.x * direction.x +
