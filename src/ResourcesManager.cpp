...
Texture* ResourcesManager::loadTexture(std::string name, std::string path,
                                       GLint internalformat, bool flip) {
  auto isPresented =
      std::find_if(textures.begin(), textures.end(),
                   [&path](Texture* t_) { return t_->path == path; });
...
Shader* ResourcesManager::loadShader(const char* vertexPath,
                                     const char* fragmentPath) {
  Shader* newSh = new Shader(vertexPath, fragmentPath);
  return newSh;
}

Material* ResourcesManager::loadMaterial(std::string name_, float shininess_,
                                         std::vector<Texture*>& textures_,
                                         glm::vec3 blendColor_) {
  auto isPresented =
      std::find_if(materials.begin(), materials.end(),
                   [&name_](Material* m_) { return name_ == m_->name; });
  if (isPresented != materials.end()) {
    return *isPresented;
  }
  if (std::find_if(textures_.begin(), textures_.end(), [](Texture* t_) {
        return t_->type == "texture_diffuse";
      }) == textures_.end()) {
    textures_.push_back(_defaultDiffuse);
  }
  if (std::find_if(textures_.begin(), textures_.end(), [](Texture* t_) {
        return t_->type == "texture_specular";
      }) == textures_.end()) {
...
void ResourcesManager::processNode(aiNode* node, const aiScene* scene,
  ...
  for (unsigned int i = 0; i < node->mNumMeshes; i++) {
    ...
    for (unsigned int i = 0;
      ...
      Texture* text = loadTexture("texture_diffuse", path + str.C_Str(),
                                  GL_SRGB_ALPHA, flip);
    ...
    for (unsigned int i = 0;
      ...
      Texture* text =
          loadTexture("texture_specular", path + str.C_Str(), GL_RGB, flip);
      textures.push_back(text);
    }
    for (unsigned int i = 0;
         i < material->GetTextureCount(aiTextureType_NORMALS); i++) {
      aiString str;
