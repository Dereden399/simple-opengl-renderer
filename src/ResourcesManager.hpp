...
public:
    ...
    Texture* loadTexture(std::string name, std::string path, GLint internalformat, bool flip = false);
...
private:
    ...
    void processNode(aiNode *node, const aiScene *scene, std::vector<Model::MeshMaterialPair>& pairs, std::string path, std::string name, bool flip = false);
};
